...
export function remoteProvider(url: string, key: string, model: string): Provider {
  const endpoint = new URL(url);
  if (endpoint.protocol !== 'https:' || endpoint.username || endpoint.password || endpoint.hash || endpoint.search) {
    throw new Error('UPSTREAM_URL must be a fixed HTTPS URL without credentials, query, or fragment');
  }
  return async messages => {
    ...
    try {
      while (true) {
        ...
        if (bytes > 256_000) throw new Error('Provider response exceeded limit');
        chunks.push(value);
      }
    } finally { await reader.cancel(); }
    const result = z.object({ choices: z.array(z.object({ message: z.object({
      content: z.string().max(32_000), tool_calls: z.never().optional(),
    }) })).min(1).max(1) }).parse(JSON.parse(Buffer.concat(chunks).toString('utf8')));
