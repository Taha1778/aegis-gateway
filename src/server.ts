...
if (
  ...
  (!process.env.GATEWAY_API_KEY || !process.env.ADMIN_API_KEY)
) {
...
if (upstream.some(Boolean) && !upstream.every(Boolean))
  throw new Error("Configure all three UPSTREAM variables");
const app = await buildApp({
  database: process.env.DATABASE_PATH ?? "data/aegis.sqlite",
  apiKey: process.env.GATEWAY_API_KEY,
  adminKey: process.env.ADMIN_API_KEY,
  provider: upstream.every(Boolean)
    ? remoteProvider(upstream[0]!, upstream[1]!, upstream[2]!)
    : undefined,
  logger: true,
});
