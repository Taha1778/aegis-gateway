...
export class Store {
  ...
  record(stage: string, action: string, rules: string[], duration: number) {
    ...
    this.db.exec(
      "DELETE FROM events WHERE rowid NOT IN (SELECT rowid FROM events ORDER BY rowid DESC LIMIT 1000)",
    );
    return id;
  }
  events() {
    return this.db
      .prepare("SELECT * FROM events ORDER BY rowid DESC LIMIT 100")
      .all();
  }
  stats() {
    return this.db
      .prepare("SELECT action, count(*) AS count FROM events GROUP BY action")
      .all();
  }
  ...
  approvals() {
    return this.db
      .prepare(
        `SELECT id, created_at, expires_at, tool, preview,
      CASE WHEN expires_at <= ? THEN 'expired' ELSE status END AS status
      FROM approvals ORDER BY rowid DESC LIMIT 100`,
      )
      .all(Date.now());
  }
  decide(id: string, status: "approved" | "denied") {
    return this.db
      .prepare(
        "UPDATE approvals SET status = ? WHERE id = ? AND status = 'pending' AND expires_at > ? RETURNING id",
      )
      .get(status, id, Date.now());
  }
  consume(id: string, call: ToolCall, principal: string) {
    return this.db
      .prepare(
        `UPDATE approvals SET status = 'consumed' WHERE id = ? AND call_hash = ?
      AND principal = ? AND status = 'approved' AND expires_at > ? RETURNING id`,
      )
      .get(id, callHash(call), principal, Date.now());
  }
  close() {
    this.db.close();
  }
